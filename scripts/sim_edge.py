#!/usr/bin/env python3
"""Simulate a real HVAC node from a laptop without a Pico.

sim_edge.py drives a USB-to-TTL radio with the same sealed request and
AT+SEND cadence as the RP2350 firmware. Each requested setpoint is sealed
with the OPERATION COLD IRON field key so the BMS gateway accepts the
request and answers with a sealed SETPOINT command. Students can validate
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the HVAC edge simulator"
    ) from exc
...
def _tick(ser, args, seq):
    """Transmit one simulated sealed HVAC setpoint request.
    ...
    None
    """
    request = args.setpoint.to_bytes(2, "little")
    envelope = field_crypto.seal_field_frame(request)
    cmd = f"AT+SEND={HUB_ADDRESS},{len(envelope)},{envelope}\r\n"
    ser.write(cmd.encode("utf-8"))
    ser.flush()
    print(f"[node {args.node} seq {seq}] sealed setpoint {args.setpoint}",
          flush=True)
