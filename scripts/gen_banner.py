#!/usr/bin/env python3
"""Generate the hvac-automation-node.png banner (1500x1500, pure stdlib).
...
def _draw_titles(banner, colors) -> None:
    ...
    None
    """
    banner.text("HVAC AUTOMATION NODE", 4,
                colors["gold"], banner.W // 2, 90)
    banner.text("OPERATION IRON LUNG", 10, (0x0B, 0x10, 0x26),
                banner.W // 2 - 6, 176)
    banner.text("OPERATION IRON LUNG", 10, colors["white"],
                banner.W // 2, 182)
    banner.text("RP2350  PICO 2", 5, colors["cyan"], banner.W // 2, 560)
    banner.text("-=[ BMS DEFENSE + PERSISTENCE LAB ]=-", 3, colors["dim"],
                banner.W // 2, 660)
...
def _draw_features(banner, colors) -> None:
    ...
    None
    """
    banner.text("HVAC_AUTOMATION_NODE", 5, colors["cyan"],
                banner.W // 2, 750)
    banner.text("BAFFLE  OVERRIDE BUTTON", 4, colors["silver"],
                banner.W // 2, 900)
    banner.text("IR REMOTE  DHT11 CLIMATE", 3, colors["silver"],
                banner.W // 2, 990)
    banner.text("XCHACHA20-POLY1305  ARGON2ID", 4, colors["dim"],
                banner.W // 2, 1120)
    banner.text("PERSISTENCE  ROOTKIT", 4, colors["gold"],
                banner.W // 2, 1260)
    banner.text("FAIL-SAFE  AUTHENTICATED STATE", 4, colors["gold"],
                banner.W // 2, 1350)
    banner.text("-=[ OPERATION IRON LUNG  /  RP2350 ]=-", 3, colors["dim"],
                banner.W // 2, 1440)
...
def main() -> int:
    ...
    out = Path(__file__).resolve().parent.parent / "hvac-automation-node.png"
    banner.save(out)
