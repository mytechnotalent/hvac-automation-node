#!/usr/bin/env python3
"""Inject a forged HVAC SETPOINT into a node to teach authentication.

Spoof.py is the classroom data-injection demonstration. It claims the
BMS gateway address on the physical transceiver and transmits a crafted
SETPOINT command to the victim node. Against the naive node the forged
command moved the baffle; against the hardened node the same attack fails
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the HVAC spoof tool"
    ) from exc
...
def _add_forge_args(parser):
    """Add the forged setpoint and attack-mode options.
    ...
    parser.add_argument("--setpoint", type=int, default=DEFAULT_SETPOINT,
                        help="Forged setpoint in tenths of a degree Celsius")
...
def _command_body(seq, setpoint):
    """Build a forged SETPOINT body of sequence, value, and random tag.
    ...
    setpoint : int
        Forged setpoint in tenths of a degree Celsius.
    ...
    return head + setpoint.to_bytes(2, "little") + secrets.token_bytes(TAG_LEN)
...
def _frame(args):
    ...
    return _forged_envelope(_command_body(args.seq, args.setpoint))


def main():
    """Inject the forged HVAC SETPOINT under the gateway address.
    ...
    print(f"Injected forged {args.mode} setpoint {args.setpoint} to node "
          f"0x{args.victim}")
