 ...
 * status LED, manual override button, baffle servo, infrared decoder,
...
static void assert_lamps(int red, int yellow, int green) {
    ...
    TEST_ASSERT_EQUAL_INT(yellow,
                          mock_gpio_get(HVAC_YELLOW_LED_PIN));
    TEST_ASSERT_EQUAL_INT(green,
                          mock_gpio_get(HVAC_GREEN_LED_PIN));
}
 ...
 * @return void
 */
static void assert_show(hvac_led_state_t state, int red, int yellow,
                        int green) {
...
static void test_status_led_show(void) {
    ...
    assert_show(HVAC_OFF, 0, 0, 0);
    assert_show(HVAC_ALARM, 1, 0, 0);
    assert_show(HVAC_OVERRIDE_PENDING, 0, 1, 0);
    assert_show(HVAC_NOMINAL, 0, 0, 1);
}
 ...
 * @return void
 */
static void test_override_pressed(void) {
    reset_pc();
    TEST_ASSERT_TRUE(override_init());
    gpio_put(HVAC_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(override_pressed());
    gpio_put(HVAC_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(override_pressed());
}
 ...
 * @return void
 */
static void test_override_consume(void) {
    reset_pc();
    override_init();
    gpio_put(HVAC_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(override_consume_press());
    gpio_put(HVAC_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(override_consume_press());
    TEST_ASSERT_FALSE(override_consume_press());
}
 ...
 * @return void
 */
static void override_release(void) {
    gpio_put(HVAC_BUTTON_PIN, true);
    override_consume_press();
}
 ...
 * @return void
 */
static void test_override_debounce(void) {
    reset_pc();
    override_init();
    gpio_put(HVAC_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(override_consume_press());
 ...
 * @return void
 */
static void test_override_debounce_elapsed(void) {
    reset_pc();
    override_init();
    gpio_put(HVAC_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(override_consume_press());
    override_release();
    mock_timer_set_us(mock_timer_now_us() + OVERRIDE_DEBOUNCE_US);
    gpio_put(HVAC_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(override_consume_press());
}

/**
 * @brief Verify override_reset clears the debounce state.
 *
 * @param void No parameters.
 * @return void
 */
static void test_override_reset(void) {
    reset_pc();
    override_init();
    gpio_put(HVAC_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(override_consume_press());
...
static void test_servo_init(void) {
    ...
    slice = pwm_gpio_to_slice_num(HVAC_SERVO_PIN);
...
static void test_ir_init(void) {
    ...
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[HVAC_IR_PIN]);
}
...
static void run_indicator_tests(void) {
    ...
    RUN_TEST(test_override_pressed);
    RUN_TEST(test_override_consume);
    RUN_TEST(test_override_debounce);
    RUN_TEST(test_override_debounce_elapsed);
    RUN_TEST(test_override_reset);
}
