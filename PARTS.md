# OPERATION IRON LUNG - Hardware Parts and Products
...
The hardware below builds the OPERATION IRON LUNG HVAC automation node and its
classroom labs. It is the same breadboard as Acts I to III, so every peripheral
in the Embedded Hacking kit is **required**: the servo is the baffle actuator, the
button is the manual override, the infrared eye is the local override remote
(OPEN, CLOSE, and CLEAR), the LEDs annunciate HVAC ALARM, OVERRIDE PENDING, and
HVAC NOMINAL, the DHT11 is the room climate sensor, the 1602 LCD is the BMS
status and setpoint readout, and the RYLR998 radio carries the sealed SETPOINT
...
reserved-sector persistence marker, watch the rootkit masking branch, trigger the
logic bomb, and step past the anti-debug trap, so it is effectively required for
Lab 3.
...
| ---- | ------------- | ----- |
| Legitimate sealed SETPOINT command loop | **2** | 1x HVAC-node RYLR998 (on the Pico) + 1x gateway RYLR998 (USB-to-TTL) |
...
- [1x DHT11 temperature and humidity sensor (room climate sensor)](https://www.amazon.com/s?k=dht11+temperature+and+humidity+sensor)
...
- [1x Push button (tactile switch, the manual override)](https://www.amazon.com/s?k=tactile+push+button+assortment)

### Actuator and infrared control surface

- [1x SG90 servo motor (the baffle actuator)](https://www.amazon.com/s?k=sg90+micro+servo+motor)
...
- [1x Infrared (IR) remote controller (NEC-compatible local override remote)](https://www.amazon.com/s?k=arduino+ir+remote+control)

### LoRa radios and serial adapters

- [3x RYLR998 LoRa module with antenna (HVAC node, gateway, and attacker)](https://www.amazon.com/s?k=rylr998+lora+module)
...
| ---------- | ---- | ----- |
| DHT11 room climate sensor | GP4 | 10K pull-up required |
| 1602 LCD SDA | GP2 | I2C1 |
| 1602 LCD SCL | GP3 | I2C1, address 0x27 |
| RYLR998 TX (Pico RX) | GP9 | UART1 |
| RYLR998 RX (Pico TX) | GP8 | UART1 |
| Infrared local override remote | GP5 | VS1838B, active low, OPEN/CLOSE/CLEAR |
| Baffle servo signal | GP14 | PWM, 50 Hz, 1000uF bulk on the 5V rail |
| Red HVAC ALARM LED | GP16 | 220-330 Ohm to ground |
| Yellow OVERRIDE PENDING LED | GP17 | 220-330 Ohm to ground |
| Green HVAC NOMINAL LED | GP18 | 220-330 Ohm to ground |
| Manual override button | GP15 | Active low, internal pull-up, requests authorization |
...
- **Argon2id** (RFC 9106) derives the 256-bit field key from a provisioned
  ...
  fits the RP2350 SRAM budget; raise it on the BMS gateway.
- **XChaCha20-Poly1305** seals every request and command with a 192-bit nonce and
  ...
  never moves the baffle.

Act IV adds stateful security on top of the sealed wire:
...
- **Authenticated state tag.** A keyed tag over the authorization record means a
  debugger that flips the SRAM `granted` boolean is rejected before the baffle
  moves.
- **Guarded SETPOINT set and safe band.** The only accepted command is
  `HVAC_COMMAND_SETPOINT` (`0x01`), and the only accepted setpoints are in the
  `50` to `350` tenths band (5.0 C to 35.0 C).
...
The FROSTLINE implant is compiled only under `SANDBOX_ONLY`. It writes a one-time
`0xC7` marker into the reserved sector `0x103FF000` on first run, re-installs the
beacon and the logic bomb on every later boot, beacons every 8 ticks with the
magic `DE AD BE EF`, arms on the 8-byte magic `FROSTLNE`, closes the baffle 3
ticks later, masks its own beacon from the LCD and the log while the beacon still
transmits, and checks CoreDebug `DHCSR` at `0xE000EDF0` to hide from a probe. It
is real in technique and inert in effect: it touches only its own pins, its own
radio frame, and its reserved sector on the same chip, on your own breadboard,
with no network and no host impact.
